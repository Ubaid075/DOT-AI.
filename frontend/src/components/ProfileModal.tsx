...
import { User, Transaction, Review, CreditRequest, GenerationHistoryItem } from '../types';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70" onClick={handleClose}>
      ...
      >
        ...
        <div className="flex-grow overflow-y-auto pr-2">
            ...
            {activeTab === 'requests' && isEditable && <CreditRequestsTab />}
...
const ProfileTab: React.FC<{user: User, isEditable: boolean, onSave: () => void}> = ({ user, isEditable, onSave }) => {
    ...
    const handleRemoveAvatar = () => {
        updateUserProfile(user.id, { avatar: "" }); // Send empty string to remove
        setNewAvatar(''); 
    };
...
const HistoryTab: React.FC = () => {
    ...
    useEffect(() => {
        const fetchHistory = async () => {
            try {
                const { data } = await api.get<GenerationHistoryItem[]>('/users/me/history');
                setGenerations(data);
            } catch (error) {
    ...
    if (isLoading) return <div className="text-center py-12 text-gray-500 dark:text-gray-400">Loading history...</div>;
...
const TransactionsTab: React.FC = () => {
    ...
    if (isLoading) return <div className="text-center py-12 text-gray-500 dark:text-gray-400">Loading transactions...</div>;
...
const CreditRequestsTab: React.FC = () => {
    ...
    useEffect(() => {
        const fetchRequests = async () => {
            try {
                const { data } = await api.get<CreditRequest[]>('/users/me/credit-requests');
                setRequests(data);
            } catch (error) {
    ...
    if (isLoading) return <div className="text-center py-12 text-gray-500 dark:text-gray-400">Loading requests...</div>;
...
const ReviewTab: React.FC<{ user: User }> = ({ user }) => {
    ...
    useEffect(() => {
        const checkExistingReview = async () => {
            try {
                // Assuming a /reviews endpoint that can filter by user might be better,
                // but for now, we can fetch all and find.
                ...
                // Don't toast here, it's not a critical failure
                console.error("Could not check for existing reviews.");
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Optimistically show the new review
        setExistingReview({ 
            id: Date.now(), 
            userId: user.id, 
            name: user.name, 
